...
from attention_model.blocks import attention_block



def densenet169_attention_model(img_rows, img_cols, color_type=1, nb_dense_block=4, growth_rate=32, nb_filter=64, reduction=0.5,
    ...
    for block_idx in range(nb_dense_block - 1):
        ...
        print("dense block", block_idx, x.shape)
        for i in range(block_idx+1):
            x = attention_block(x, encoder_depth=3-block_idx)  # bottleneck 7x7
            #print("attention block", i, x.shape)
        # Add transition_block
        x = transition_block(x, stage, nb_filter, compression=compression, dropout_rate=dropout_rate,
                             weight_decay=weight_decay)
        #print("transition block", block_idx, x.shape)
    ...
    model = Model(img_input, x_fc, name='densenet_attention')
    '''
...
if __name__=="__main__":
    model = densenet169_attention_model(224, 224)
    model.summary()
